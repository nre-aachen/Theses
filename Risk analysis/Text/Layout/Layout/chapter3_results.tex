	\chapter{Results}\label{cha:results}
		...
		\end{figure}
		The nature of the prior uncertainties is reflected by the visualization using information entropy (see Figure \ref{fig:ML1} (a)). High entropies are found at and near the possible locations of layer boundaries. This effect is amplified by the offset uncertainty in the footwall, where the highest information entropies are found.\\
		Regarding the different mechanisms of maximum trap volume control, it can be observed (see Figure \ref{fig:ML1} (c)) that all four relevant mechanisms occurred. The dominant factor is the anticlinal spill point (labeled "SPILL POINT"), accounting for more than 60\% of model realizations. It is followed by cross-fault leakage due to juxtaposition with formations underlying ("LEAK UNDER", ~20\%) and formations overlying ("LEAK OVER", ~13\%) the seal in the hanging wall. Leakage due to stratigraphical connections to seal-overlying units ("STRAT"), i.e. stratigraphical breaches of the seal, were registered to be decisive in only about 3\% of realizations. A satisfactory reliability of model algorithms and computation is indicated by the lack of unrecognized mechanisms (labeled "UNCLEAR", see Figure \ref{fig:ML1} (c)).\\
		Recoverable oil volumes were calculated for each model iteration and plotted as a probability distribution in Figure \ref{fig:M_prior} (a). It can clearly be seen that while a large portion of the distribution is found in highly positive volumes, forming a normal distribution shape around 3.8 million $m³$, there also a high probability of complete failure, i.e. a maximum trap volume of zero. This results in a bimodal characterization, with low probability of the $ROV$ to be found between zero and three million $m³$.\\
		Consequently, applying the custom loss function on this distribution results in divergent Bayes actions for the different risk-affine actors (see Figure \ref{fig:M_prior} (b)). Although all actors find their estimates to be positive, only the risk-neutral to risk-friendliest actors come close to the positive mode of the distribution. Risk-averse individuals bid on significantly lower estimates. It can also be observed, that expected losses seem to be generally lower for risk-affine actors. The different Bayes actions have apparently been torn apart by the bimodality of the $ROV$ probability distribution.
		...
		\end{table}
		For the simulation of this first posterior model, thickness likelihoods for the three central layers were included into the probabilistic model. These were defined in a way that reinforced the probabilities of the relevant layers to be significantly thick (see Table \ref{tab:ML1_likelihoods}). 
		\begin{figure}[p!]
			\centering
			\includegraphics[width=1\textwidth]{Figures/ML1}
			\caption{....}\label{fig:ML1}
		\end{figure}		
		Respective results are summarized in Figure \ref{fig:ML1}. Considering the changes in information entropy, it can be seen that the uncertainty about layer interface positions in $z$ was clearly reduced in the footwall. Information entropy decreased also in the middle section of the hanging wall, but remained high in upper and lower parts, presumably due to the remaining influence of the offset uncertainty.\\
		Regarding trap volume control (Figure \ref{fig:ML1} (c)), only spill point-related migration and cross-fault leakage due to juxtaposition with permeable layers below the seal in the hanging wall remain relevant in the posterior model. As no occurrences of leakage through overlying layers was observed, the two other mechanisms increased in frequency, while staying proportionally the same to one another. It can be presumed, that by introducing the respective likelihoods, a high thickness of the seal was ensured, as can also be recognized in Figure \ref{fig:ML1} (b). This not only made stratigraphic leakage in this model impossible, but also enforced a favorable $SSF$. A minor emergence of unrecognized control factors (label: "UNCLEAR") hints at a singular cases in which the model was possibly not correctly computable or the structural analysis failed for a different reason.\\
		The bulk that accounted for the positive mode in the prior $ROV$ distribution was raised and shifted laterally to higher values, while the probability of failure ($ROV = 0$) was diminished significantly. This can presumably be explained by the disappearing of leakage to overlying formations as a trap control mechanism, which was programmed to always lead to complete failure of the trap volume.\\
		As the probability of failure and thus the bimodality of the probability distribution disappeared, all Bayes actions were shifted to higher estimates, most significantly those of the two risk-averse individuals (see Figure \ref{fig:ML1} (e). There is thus clear convergence of the different points of minimum expected loss, which was also lowered overall. It is clear that the posterior state of knowledge, provided by the updated model, encourages even the most risk-averse decision-maker to expect recoverable oil volumes from 3.5 million m$^3$ upwards.
		
		\subsection{Posterior model II: Likely thin reservoir formation}%ML3
		For the following model, thickness likelihoods were defined in a way, that reinforced the probability of a very thick seal, but at the same time made restricted the reservoir formation to a likely significantly thinner reservoir (around 50 m) (see Table in Appendix). The results (see Appendix) are very similar to those of model I above. This can be explained by the influence of leak and spill point. As the shape of the general structure itself is not defined to be uncertain, the position of the anticlinal spill point relative to the fault top remains mostly unchanged, irrespective of layer thickness variations. Thus, reservoir thickness is a secondary factor to trap volume control. Given all other parameters are fixed, the maximum fill horizon is most likely the same for a 300 m as for a 50 m thick reservoir unit. The thickness of the reservoir only becomes respectively significant, if it thin enough to set the layer bottom above the leak and spill point depth in the trap section. Ultimately, the spill point appears to be the primariy limiting factor to maximum trap fill. It can furthermore be derived from this that the maximum trap volume capacity, as defined by the given structural features, is already represented in the $ROV$ probability distribution of the prior model. Examining this in Figure \ref{fig:ML1} (d), it seems that this overall maximum volume is found at around 5.7 millon m$3$. Higher values were not observed in any other model realization and are presumed to be unattainable due to the restriction given by the spill point.
		...
		\end{figure}
		Results from this third model show that in contrast to the reservoir thickness, seal thickness is a primary and decisive factor for trap volume. Using likelihoods which favor a thick reservoir, but only a thin seal of about 50 m thickness, leakage due to juxtaposition with seal-overlying formations across the fault becomes the primary mechanism for trap volume control (see Figure \ref{fig:ML2} (c). It can recognized in Figure \ref{fig:ML2} (b) that while uncertainty was reduced clearly in the footwall, it remains high in the hanging wall. The state of information entropy in furthermore suggests that the seal layer in the appeared often significantly deeper hanging wall than in the footwall. Combined with a generally very thin seal, this allowed for the occurrence of high displacement to seal thickness ratios, i.e. high Shale Smear Factors. Thus, a frequenting surpassing of the critical $SSF$ and subsequent fault seal breaching was enabled. It is additionally apparent in Figure \ref{fig:ML2} (b), that while fault-related leakage has become predominant, control by the spill point has become widely irrelevant in this posterior model.\\	%It is suggested by the footwall information entropy in Figure \ref{label} (b), that the general shape of the structure has been flattened.
		A dramatically higher probability of complete trap failure is induced by the high frequency of cross-fault leakage to overlying formations. Consequently, the bulk of the posterior $ROV$ probability distribution is dropped to a volume of zero, as depicted in Figure \ref{fig:ML2} (d). Any positive trap volumes have become highly improbable.\\
		Bayes actions changed accordingly. Given this posterior $ROV$ distribution, all actors find their minimum of expected loss at $ROV = 0$, which equals the decision to "take no action" (see Figure \ref{fig:ML2} (e)). It has to be pointed out that all Bayes actions are found at the same point, due to the lack of negative values, into which more risk-averse actors would presumably move, if given the possibility. Compared to the prior model, expected losses were decreased overall. 
